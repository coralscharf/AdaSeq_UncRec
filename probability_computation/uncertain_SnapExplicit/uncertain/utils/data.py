...
class Data(LightningDataModule):
    ...
    def __init__(self, data, users_on_test=None, test_ratio=0.2, val_ratio=0.2, implicit=False,
        ...
        indices_to_drop = data.index[data.user.isin(length.index[length < 5])]
        ...
        indices_to_drop = data.index[data.item.isin(length.index[length < 5])]
        if not indices_to_drop.empty:
            data.drop(indices_to_drop, axis=0, inplace=True)
        ...
        self.item = pd.concat([self.item, empty]).sort_index().fillna(0)
