    ...
    def rank_dist_with_tiebreaking_one_by_one(self, k) -> torch.Tensor:
        ...
        for i in tqdm(range(self.universe_len)):
            ...
            for n_ties in range(1, k):
                tmp = conditioned_matrix[n_ties, :, :].clone()
...
def run_small_test3():
    ...
    score_probabilities = torch.from_numpy(np.array([[0.4, 0.6, 0.0],
                     [0.2, 0.8, 0.0],
                     [0.1, 0.4, 0.5]]))
    score_values = torch.from_numpy(np.array([[2, 4, 10],
                     [1, 4.5, 7],
                     [0.5, 3, 5]]))
    rank_dist = RankDist(score_probabilities, score_values, device)
...
def run_small_test4():
    ...
    score_probabilities = torch.from_numpy(np.array([[0.45, 0.55],
                     [0.4, 0.6],
                     [0.6, 0.4]]))
    score_values = torch.from_numpy(np.array([[1, 5],
                     [0.5, 4.5],
                     [3, 4]]))
    rank_dist = RankDist(score_probabilities, score_values, device)
    result = rank_dist.rank_dist(3)
    ...
    score_probabilities = torch.from_numpy(np.array([[0.0, 1],
                     [0.4, 0.6],
                     [0.6, 0.4]]))
    score_values = torch.from_numpy(np.array([[1, 5],
                     [0.5, 4.5],
...
def rank_dist_runtime_test(scores_num=10, score_options=[5, 10, 15, 20, 25, 50, 100],
                           uni_options=[1000,5000,10000], k_options=[10,50,100],
    ...
    combinations = []
    score_text_for_file_name = "multiple_scores" if score_options is not None \
        else f"scores_num_{scores_num}"
    if score_options is not None:
        for scores_option_num in score_options:
            for uni_size in uni_options:
                for top_k_size in k_options:
                    combinations.append((scores_option_num, uni_size, top_k_size))
    else:
        if k_options is not None:
            for top_k_size in k_options:
                for uni_size in range(min_uni_size, max_uni_size + uni_size_jump, uni_size_jump):
                    combinations.append((scores_num, uni_size, top_k_size))
    ...
    results = {}
    for num_of_scores, uni_size, top_k_size in combinations:
        print("*"*20)
        print(f'Starting testing with num_of_scores={num_of_scores}, '
              f'uni_size={uni_size}, top_k_size={top_k_size}')
        total_time = 0
        for i in range(num_repeats):
            score_probabilities, score_values = generate_universe(uni_size, num_of_scores)
            start = time.time()
        ...
        results[(num_of_scores, uni_size, top_k_size)] = avg_runtime
        print(f'num_of_scores={num_of_scores}, uni_size={uni_size}, top_k_size={top_k_size}. '
              f'Took {avg_runtime} seconds on average')

    # Write results to CSV
    with open(f'./results/rankdist_{rank_dist_type}_{score_text_for_file_name}_{num_repeats}_repeats_runtime',
              'w', newline='') as csvfile:
        fieldnames = ['scores_num', 'uni_size', 'top_k_size', 'avg_runtime']
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

        writer.writeheader()
        for (num_of_scores, uni_size, top_k_size), avg_runtime in results.items():
            writer.writerow({'scores_num': num_of_scores, 'uni_size': uni_size,
